...
function writeCityToUrl(index,categoryKey){if(!citySections.length)return;const category=(categoryKey||"all").toLowerCase();const url=new URL(window.location.href);const cityName=(citySections[index]?.getAttribute("data-city")||"").trim();if(!cityName)return;url.searchParams.set("city",cityName===WHOLE_WORLD_KEY?"all":cityName);if(category!=="all")url.searchParams.set("category",category);else url.searchParams.delete("category");window.history.replaceState(null,"",url.toString());}
