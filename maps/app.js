...
function searchCoordsFromHref(href){const m=/\/maps\/search\/(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)/.exec(String(href||""));if(!m)return null;const lat=parseFloat(m[1]);const lng=parseFloat(m[2]);if(!Number.isFinite(lat)||!Number.isFinite(lng))return null;return {lat,lng};}
