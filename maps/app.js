...
function setupMasonry(){[...document.querySelectorAll("section.city")].forEach(city=>{let cats=[...city.children].filter(el=>el.tagName==="SECTION"&&el.classList.contains("category"));if(!cats.length)return;const cafeBakeryCards=cats.filter(card=>{const key=(card.getAttribute("data-category")||"").toLowerCase();return key==="cafe"||key==="bakery";});if(cafeBakeryCards.length){const primary=cafeBakeryCards.find(card=>(card.getAttribute("data-category")||"").toLowerCase()==="cafe")||cafeBakeryCards[0];const primaryList=primary.querySelector(":scope > ul");primary.setAttribute("data-category",combinedCafeBakeryKey);const heading=primary.querySelector(":scope > h5");if(heading)heading.textContent="☕🥐 Cafes & Bakeries";cafeBakeryCards.forEach(card=>{if(card===primary)return;const list=card.querySelector(":scope > ul");if(primaryList&&list){[...list.children].forEach(li=>primaryList.appendChild(li));}card.remove();});cats=[...city.children].filter(el=>el.tagName==="SECTION"&&el.classList.contains("category"));}const comboIdx=cats.findIndex(el=>(el.getAttribute("data-category")||"").toLowerCase()===combinedCafeBakeryKey);const artIdx=cats.findIndex(el=>(el.getAttribute("data-category")||"").toLowerCase()==="art");if(comboIdx!==-1&&artIdx!==-1&&comboIdx!==artIdx-1){const [combo]=cats.splice(comboIdx,1);const target=Math.max(0,cats.findIndex(el=>(el.getAttribute("data-category")||"").toLowerCase()==="art"));cats.splice(target,0,combo);}const nextArtIdx=cats.findIndex(el=>(el.getAttribute("data-category")||"").toLowerCase()==="art");const barIdx=cats.findIndex(el=>(el.getAttribute("data-category")||"").toLowerCase()==="bar");if(nextArtIdx!==-1&&barIdx!==-1){const tmp=cats[nextArtIdx];cats[nextArtIdx]=cats[barIdx];cats[barIdx]=tmp;}const wrap=document.createElement("div");wrap.className="category-masonry";cats.forEach(c=>wrap.appendChild(c));city.appendChild(wrap);});}
